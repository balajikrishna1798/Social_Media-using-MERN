...
const Forms = (props: any) => {
  ...
  const post = useAppSelector((state: any) =>
    props.currentId
      ? state.posts.posts.find((p: any) => p._id === props.currentId)
      : null
  );
  ...
  const handleSubmit = (e: any) => {
    ...
    for (const tags of postdata.tags) postData.append("tags", tags);
    postData.append("selectedFile", postdata.selectedFile[0]);

    if (props.currentId) {
      dispatch(
        updatePost({ id: props.currentId, postData: postdata, navigate })
      );
    } else {
    ...
    <div className="container">
      ...
      >
        <div className="text-center fw-bold mb-2">
          {!props.currentId ? "Creating" : "Editing"} a Memory
        </div>
        <input
          type="text"
          placeholder="Name"
